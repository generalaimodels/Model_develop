...
class CustomTrainerForHemanth:
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            optimizer_cls, optimizer_kwargs = CustomTrainerForHemanth.get_optimizer_cls_and_kwargs(self.args, opt_model)
    ...
    def _inner_training_loop(
        ...
        def format_box(text, width=60, color=None):
            lines = text.split("\n")
            max_line_length = max(len(line) for line in lines)
            box_width = max(width, max_line_length + 4)
            
            border = "+" + "-" * (box_width - 2) + "+"
            padding = " " * ((box_width - max_line_length - 4) // 2)
            
            formatted_lines = [
                f"| {padding}{line}{' ' * (box_width - len(line) - len(padding) -7)}{padding}|"
                for line in lines
            ]
            
            box = "\n".join([border] + formatted_lines + [border])
