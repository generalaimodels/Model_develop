...
is_lomo_available=False

class TrainerForHemanth:
    """
    ...
    def __init__(
        ...
        if (
            ...
            and isinstance(train_dataset, IterableDataset)
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            optimizer_cls, optimizer_kwargs = TrainerForHemanth.get_optimizer_cls_and_kwargs(self.args, opt_model)
    ...
    def _inner_training_loop(
        ...
        # Train!
        logger.info(border)
        logger.info("|{:^56}    |".format("Running Training"))
        logger.info(border)
        logger.info(f"| {padding}Num examples                        : {num_examples:,}     |")
        logger.info(f"| {padding}Num Epochs                          : {num_train_epochs:,}          |")
        logger.info(f"| {padding}Batch size per device               : {self.args.per_device_train_batch_size:,}        |")
        
        if self.args.per_device_train_batch_size != self._train_batch_size:
            logger.info(f"| {padding}Training with DataParallel adjusted : {self._train_batch_size:,}         |")
        
        logger.info(f"| {padding}Total train batch size              : {total_train_batch_size:,}        |")
        logger.info(f"| {padding}Gradient Accumulation steps         : {args.gradient_accumulation_steps}          |")
        logger.info(f"| {padding}Total optimization steps            : {max_steps:,}        |")
        logger.info(f"| {padding}Trainable parameters                : {get_model_param_count(model, trainable_only=True):,} |")
        logger.info(border)
    ...
    def evaluation_loop(
        ...
        padding = " " * 2
        logger.info(border)
        logger.info(f"|    {'Running ' + description:^40}|")
        logger.info(border)
        if has_length(dataloader):
            num_examples_str = f"{self.num_examples(dataloader):<28}"
        else:
            num_examples_str = "Unknown"
        
        logger.info(f"|{padding}Num examples :{num_examples_str}|")
        logger.info(f"|{padding}Batch size   :{batch_size:<28}|")
        logger.info(border)
        # logger.info(f"\n***** Running {description} *****")
        # if has_length(dataloader):
    ...
    def prediction_loop(
        ...
        padding = " " * 2
        logger.info(border)
        logger.info(f"|{padding}{'Running ' + description:^32}{padding}|")
        logger.info(border)
        logger.info(f"| {padding}Num examples : {num_examples:<22} {padding}|")
        logger.info(f"| {padding}Batch size   : {batch_size:<22} {padding}|")
        logger.info(border)
        # logger.info(f"\n***** Running {description} *****")
