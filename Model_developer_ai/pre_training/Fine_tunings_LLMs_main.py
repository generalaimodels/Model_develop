...
def main():
    args = parse_args()
    config_args = load_arguments_from_yaml(yaml_file_path='E:/LLMS/Fine-tuning/llms-data/Fine_TuningS/fine_tuning/Fine_tuning_LLMS/Hemanth/Hemanth_LLMs/model_loder/pre_training/Fine_config_llms.yml')
    ...
    if args.dataset_name is not None:
       if isinstance(args.dataset_name, (str, Path)):
           dataset = load_and_prepare_dataset(args.dataset_name)
       elif isinstance(args.dataset_name, dict):
           dataset = load_and_prepare_dataset(args.dataset_name,streaming=False)
       elif isinstance(args.dataset_name, list):
           dataset = load_and_prepare_dataset(args.dataset_name)
       else:
           raise ValueError("Invalid dataset type. It should be a string, Path, dict, or list.")
    else:
